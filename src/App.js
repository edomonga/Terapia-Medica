...
const STATS_PREFIX = 'tm_stats_v5_'; // + username
...
// Stats storage: { [argomento]: { OTTIMO, BUONO, SUFFICIENTE, INSUFFICIENTE, total } }
...
const Btn = ({ children, onClick, variant='primary', disabled, full, style:extra={} }) => {
  ...
  const vs = {
    ...
    active:    { background:C.red,   color:C.white },
  };
...
const Toggle = ({ on, onToggle, label }) => (
  <button onClick={onToggle} style={{
    ...
    background:'none', border:`1px solid ${on ? C.red : C.rule}`,
    borderRadius:3, padding:'7px 12px', cursor:'pointer',
    fontFamily:F.mono, fontSize:10, letterSpacing:'.08em', textTransform:'uppercase',
    color: on ? C.red : C.ink4, transition:'all .15s',
  }}>
    <span style={{
      width:28, height:16, borderRadius:8, backgroundColor: on ? C.red : C.rule,
      position:'relative', display:'inline-block', transition:'background .2s', flexShrink:0,
    }}>
      <span style={{
        position:'absolute', top:2, left: on ? 14 : 2, width:12, height:12,
        borderRadius:'50%', backgroundColor:C.white, transition:'left .2s',
      }}/>
    </span>
    {label}
  </button>
...
// --- STATS SCREEN ----------------------------------------------------------
function StatsScreen({ username, onBack }) {
  const stats = loadStats(username);
  const nomeDisplay = cap(username);
  const argsConDati = argomenti.filter(a => stats[a] && stats[a].total > 0);

  const totaleRisposte = argsConDati.reduce((s, a) => s + (stats[a].total||0), 0);
  const totaleOttimi   = argsConDati.reduce((s, a) => s + (stats[a].OTTIMO||0), 0);
  const pctGlobale = totaleRisposte > 0 ? Math.round(((totaleOttimi + argsConDati.reduce((s,a)=>(stats[a].BUONO||0)+s,0)*0.75) / totaleRisposte) * 100) : 0;
    ...
    <div style={{ minHeight:'100vh', background:C.paper }}>
      ...
      <div style={{ maxWidth:760, margin:'0 auto', padding:'44px 24px' }}>
        ...
        {totaleRisposte === 0 ? (
          ...
          <>
            {/* Global summary */}
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:1, background:C.rule, border:`1px solid ${C.rule}`, marginBottom:36 }}>
              {[
                  ...
                  <div key={a} style={{ background:C.white, border:`1px solid ${C.rule}`, borderRadius:3, padding:'18px 20px', marginBottom:8 }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                        ...
                        <span style={{ fontFamily:F.mono, fontSize:12, fontWeight:600, color:barColor }}>{pctOk}% corretto</span>
                      </div>
                    </div>
                    {/* Bar */}
                    ...
                    <div style={{ display:'flex', gap:16, marginTop:10 }}>
                      {['OTTIMO','BUONO','SUFFICIENTE','INSUFFICIENTE'].map(v => (
                        <span key={v} style={{ fontFamily:F.mono, fontSize:10, color: s[v] > 0 ? VOTE[v].text : C.ink4 }}>
                          {VOTE[v].label}: {s[v]||0}
                        </span>
                      ))}
                    </div>
...
function SetupScreen({ username, onStart, onLogout, onStats }) {
  ...
  const go = () => {
    // Always shuffle real questions first
    let pool;
...
function QuizScreen({ pool:initialPool, isExam, argomentiScelti, username, onEnd, onMenu }) {
  ...
  if (done) return <ResultsScreen results={results} pool={pool} isExam={isExam} username={username} onEnd={onEnd} onMenu={onMenu} />;
...
export default function App() {
  ...
  if (screen==='stats') return <StatsScreen username={username} onBack={()=>setScreen('setup')} />;
